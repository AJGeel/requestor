...
<html lang="en" dir="ltr">
  ...
  <body>
    <div class="page">
      ...
      let getParams=function(t){let n={},e=document.createElement("a");e.href=t;let o=e.search.substring(1).split("&");for(let t=0;t<o.length;t++){let e=o[t].split("=");n[e[0]]=decodeURIComponent(e[1])}return n},interfaceVariant="cui",btnConsent=document.getElementById("consent"),btnNoConsent=document.getElementById("noConsent"),toStudy=document.getElementById("toStudy"),consent=!1,p=getParams(window.location.href);function toNextPage(){consent?location.href=`app/${interfaceVariant}.php`:alert("Unfortunately, we are not allowed to perform this study with persons who do not consent to the terms of this research. \n\nWe urge you to read through the 'Subject information for participation' letter again and decide whether you would like to consent to participating in this study.")}"0"===p.v&&(interfaceVariant="gui"),btnConsent.addEventListener("click",function(){1!=btnConsent.classList.contains("active")&&btnConsent.classList.add("active"),toStudy.classList.contains("inactive")&&toStudy.classList.remove("inactive"),btnNoConsent.classList.remove("active"),consent=!0}),btnNoConsent.addEventListener("click",function(){1!=btnNoConsent.classList.contains("active")&&btnNoConsent.classList.add("active"),0==toStudy.classList.contains("inactive")&&toStudy.classList.add("inactive"),btnConsent.classList.remove("active"),consent=!1}),toStudy.addEventListener("click",function(){toNextPage()});
      </script>
